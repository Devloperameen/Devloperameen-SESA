    ...
    return (
        <div className="overflow-hidden relative">
            ...
            <section className="relative py-20 md:py-32 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-[#0a0e27] via-[#141852] to-[#1a0a3e]" />
                {/* Animated grid pattern */}
                <div className="absolute inset-0 opacity-[0.07]" style={{ backgroundImage: 'linear-gradient(rgba(99,102,241,0.3) 1px, transparent 1px), linear-gradient(90deg, rgba(99,102,241,0.3) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
                {/* Floating glow orbs */}
                <motion.div animate={{ x: [0, 30, 0], y: [0, -20, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} className="absolute top-20 left-[10%] w-72 h-72 bg-primary/20 rounded-full blur-[100px]" />
                <motion.div animate={{ x: [0, -25, 0], y: [0, 25, 0] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} className="absolute bottom-10 right-[10%] w-80 h-80 bg-accent/20 rounded-full blur-[100px]" />
                <motion.div animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-cyan-500/10 rounded-full blur-[120px]" />

                <div className="container mx-auto px-4 relative z-10">
                    <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                        {/* Left — Text + CTA */}
                        <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="text-center lg:text-left order-2 lg:order-1">
                            {/* SESA brand badge */}
                            <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-white/[0.07] backdrop-blur-xl rounded-full border border-white/10 mb-6">
                                <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
                                <span className="text-sm font-black text-white tracking-wider">SESA</span>
                                <span className="text-sm">🌍</span>
                                <span className="text-sm font-black text-white tracking-wider">ACADEMY</span>
                                <span className="w-2 h-2 bg-cyan-400 rounded-full animate-pulse" />
                            </div>

                            <h2 className="text-3xl sm:text-4xl md:text-5xl xl:text-6xl font-black text-white leading-[1.1] mb-5">
                                {t('Your Future', 'የወደፊት ህይወትዎ')}
                                <br />
                                <span className="bg-gradient-to-r from-cyan-400 via-primary to-purple-400 bg-clip-text text-transparent">{t('Starts Here', 'ከዚህ ይጀምራል')}</span>
                                <span className="inline-block ml-2">
                                    <motion.span animate={{ rotate: [0, 15, -15, 0] }} transition={{ duration: 2, repeat: Infinity, repeatDelay: 3 }} className="inline-block text-4xl md:text-5xl">✨</motion.span>
                                </span>
                            </h2>

                            <p className="text-base sm:text-lg text-white/70 max-w-lg leading-relaxed mb-8 mx-auto lg:mx-0">
                                {t(
                                    'Join thousands of students mastering cutting-edge technology. SESA Academy blends smart education with real-world tech skills — from Ethiopia to the world.',
                                    'ሺህዎች ተማሪዎችን ይቀላቀሉ። ሴሳ አካዳሚ ስማርት ትምህርትን ከተግባራዊ ቴክ ክህሎቶች ጋር ያዋህዳል — ከኢትዮጵያ ወደ ዓለም።'
                                )}
                            </p>

                            {/* Tech skill pills */}
                            <div className="flex flex-wrap gap-2 justify-center lg:justify-start mb-8">
                                {['⚛️ React', '🟢 Node.js', '🍃 MongoDB', '🐍 Python', '▲ Next.js', '🔷 TypeScript'].map((s, i) => (
                                    <motion.span key={i} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.1 * i }}
                                        className="px-3 py-1.5 bg-white/[0.06] backdrop-blur-sm border border-white/10 rounded-full text-xs font-bold text-white/80 hover:bg-white/10 hover:border-primary/30 transition-all cursor-default">{s}</motion.span>
                                ))}
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                                <Link to="/auth?role=student">
                                    <motion.button whileHover={{ scale: 1.05, boxShadow: '0 0 30px rgba(99,102,241,0.4)' }} whileTap={{ scale: 0.95 }}
                                        className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-primary via-secondary to-accent text-white font-black rounded-2xl shadow-xl shadow-primary/25 transition-all flex items-center justify-center gap-3 text-lg">
                                        <GraduationCap className="w-6 h-6" />
                                        {t("Get Started — It's Free", 'ይጀምሩ — ነፃ ነው')}
                                        <Rocket className="w-5 h-5" />
                                    </motion.button>
                                </Link>
                                <Link to="/auth?role=instructor">
                                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                        className="w-full sm:w-auto px-8 py-4 bg-white/[0.08] backdrop-blur-md border border-white/15 text-white font-bold rounded-2xl hover:bg-white/15 transition-all flex items-center justify-center gap-2 text-lg">
                                        <Sparkles className="w-5 h-5 text-yellow-400" />
                                        {t('Teach at SESA', 'በሴሳ ያስተምሩ')}
                                    </motion.button>
                                </Link>
                            </div>
                            ...
                            <div className="flex items-center gap-6 mt-8 justify-center lg:justify-start">
                                {[
                                    { v: '5,200+', l: t('Students', 'ተማሪዎች'), c: 'text-cyan-400' },
                                    { v: '120+', l: t('Courses', 'ኮርሶች'), c: 'text-emerald-400' },
                                    { v: '98%', l: t('Success', 'ስኬት'), c: 'text-yellow-400' },
                                ].map((s, i) => (
                                    <div key={i} className="text-center">
                                        <p className={`text-xl md:text-2xl font-black ${s.c}`}>{s.v}</p>
                                        <p className="text-[11px] text-white/50 font-medium">{s.l}</p>
                                    </div>
                                ))}
                            </div>
                        </motion.div>

                        {/* Right — SESA Brand Image with Tech Frame */}
                        <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.2 }} className="order-1 lg:order-2">
                            <div className="relative">
                                {/* Glowing border effect */}
                                <div className="absolute -inset-1 bg-gradient-to-r from-cyan-500 via-primary to-purple-500 rounded-3xl blur-sm opacity-60" />
                                <div className="relative rounded-3xl overflow-hidden border-2 border-white/10 shadow-2xl shadow-primary/20">
                                    <SafeImage
                                        src="/sesa-tech-brand.png"
                                        alt="SESA Academy — Tech Students"
                                        className="w-full h-[350px] sm:h-[420px] lg:h-[500px] object-cover"
                                        wrapperClassName="absolute inset-0"
                                        fallback={
                                            <div className="w-full h-[350px] sm:h-[420px] lg:h-[500px] bg-gradient-to-br from-slate-900 via-blue-900/50 to-slate-900 flex items-center justify-center">
                                                <Monitor className="w-20 h-20 text-white/20" />
                                            </div>
                                        }
                                    />
                                    {/* Overlay gradient */}
                                    <div className="absolute inset-0 bg-gradient-to-t from-[#0a0e27]/70 via-transparent to-[#0a0e27]/20" />
                                    {/* Bottom info bar */}
                                    <div className="absolute bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-black/80 to-transparent">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h4 className="text-white font-bold text-sm">SESA 🌍 Academy</h4>
                                                    <p className="text-white/60 text-xs">{t('Bright Minds, Bright Future', 'ብሩህ አእምሮ፣ ብሩህ ወደፊት')}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-1.5">
                                                <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
                                                <span className="text-emerald-400 text-xs font-bold">{t('LIVE', 'በቀጥታ')}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
                                    className="absolute -top-3 -right-3 px-3 py-1.5 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full text-white text-xs font-black shadow-lg flex items-center gap-1">
                                    <Zap className="w-3 h-3" /> MERN Stack
                                </motion.div>
                                <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
                                    className="absolute -bottom-3 -left-3 px-3 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-full text-white text-xs font-black shadow-lg flex items-center gap-1">
                                    <Star className="w-3 h-3" /> AI & ML
                                </motion.div>
                                <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
                                    className="absolute top-1/2 -left-4 px-3 py-1.5 bg-gradient-to-r from-purple-500 to-pink-600 rounded-full text-white text-xs font-black shadow-lg flex items-center gap-1">
                                    <Globe className="w-3 h-3" /> Ethiopia 🇪🇹
                                </motion.div>
                            </div>
                        </motion.div>
                    </div>
